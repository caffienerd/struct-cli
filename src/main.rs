...
fn display_summary(path: &Path) {
    ...
    for entry in entries {
        ...
        if is_dir {
            // Directory summary - calculate TOTAL stats (all files recursively)
            let mut total_file_count = 0;
            let mut total_dir_count = 0;
            let mut total_size: u64 = 0;
            let mut total_extensions: std::collections::HashMap<String, usize> = std::collections::HashMap::new();

            // Also track VISIBLE (non-ignored) items recursively
            let mut visible_file_count = 0;
            let mut visible_dir_count = 0;
            let mut visible_size: u64 = 0;
            let mut visible_extensions: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
            let mut ignored_subdirs: Vec<(String, usize)> = Vec::new();

            // Get immediate children to track ignored subdirs
            ...
            for sub_entry in WalkDir::new(&entry_path).into_iter().filter_map(|e| e.ok()) {
                if sub_entry.file_type().is_file() {
                    ...
                    if let Some(ext) = sub_entry.path().extension() {
                        let ext_str = ext.to_string_lossy().to_lowercase();
                        *total_extensions.entry(ext_str).or_insert(0) += 1;
                    }
            ...
            println!("  {}", entry_path.canonicalize().unwrap_or(entry_path.clone()).display().to_string().bright_black());
